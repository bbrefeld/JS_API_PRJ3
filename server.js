...
function createComment(url, request) {
  ...
  // if a request exists && there is a username included && there is a comment included && there is an articleId included && the username exists in the database && the articleId exists in the database
  if (requestComment && requestComment.username && requestComment.body && requestComment.articleId && database.users[requestComment.username] && database.articles[requestComment.articleId]) {
    const comment = {
    ...
    // database.comments[1] = comment object
...
function saveDatabase(database) {
  const fs = require('fs');
  fs.writeFileSync('test.yml', database, 'utf8')
};
